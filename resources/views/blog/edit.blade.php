...
@section('title', 'Edit Blog Post - Seova')
@section('description', 'Edit and update your blog post with SEO optimization.')
...
@section('hero')
<!-- Hero Section -->
<section class="bg-white relative" aria-labelledby="edit-post-title">
    <div class="max-w-4xl mx-auto px-6 py-16 text-center">
        <a href="{{ route('posts.show', $post->slug) }}" class="inline-block text-seova-orange hover:text-seova-orange/80 mb-4 text-sm font-semibold">
            ← Back to Post
        </a>

        <h1 id="edit-post-title" class="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Edit Blog Post</h1>
        <p class="text-lg text-gray-600">Update "{{ $post->title }}"</p>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8 md:p-12">
            <form action="{{ route('posts.update', $post) }}" method="POST" class="space-y-8">
                ...
                <div>
                    ...
                    >
                    <p class="mt-1 text-sm text-gray-600">Current: <code class="text-seova-orange">{{ $post->slug }}</code></p>
                    @error('slug')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($post->featured_image)
                        <p class="mt-3 text-sm text-gray-600">Current image:</p>
                        <img src="{{ $post->featured_image }}" alt="{{ $post->title }}" class="mt-2 max-w-xs rounded-lg">
                    @endif
                ...
                <div class="border-t pt-8">
                    ...
                    <div id="faqs-container" class="space-y-4">
                        @php
                            $faqs = old('faqs') ? json_decode(old('faqs'), true) : ($post->faqs ?? []);
                        @endphp
                    ...
                    <textarea
                        ...
                        id="faqs-json"
                        class="hidden"
                    >{{ old('faqs', json_encode($post->faqs ?? [])) }}</textarea>
                    @error('faqs')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="border-t pt-8">
                    ...
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        >
                        <label for="published" class="text-sm font-semibold text-gray-900">Publish this post</label>
                    </div>
            ...
            <!-- Publish/Unpublish Actions (Outside main form) -->
            @if($post->published && $post->published_at)
                <div class="mt-6 pt-6 border-t">
                    <p class="text-sm text-gray-600 mb-4">
                        Published on: <strong>{{ $post->published_at->format('F d, Y \a\t g:i A') }}</strong>
                    </p>
            ...
            @else
                <div class="mt-6 pt-6 border-t">
                    <form action="{{ route('posts.publish', $post) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="text-sm text-seova-orange hover:text-seova-orange/80 font-semibold">
                            Publish this post
                        </button>
                    </form>
                </div>
